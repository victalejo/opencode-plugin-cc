...
Use this skill only inside the `opencode:rescue` subagent.
...
- Do not call `setup`, `review`, `adversarial-review`, `status`, `result`, or `cancel` from `opencode:rescue`.
...
- If the forwarded request includes `--model <provider/model>`, pass it through to `task` unchanged.
...
Safety rules:
- Default to write-capable opencode work in `opencode:rescue` unless the user explicitly asks for read-only behavior.
