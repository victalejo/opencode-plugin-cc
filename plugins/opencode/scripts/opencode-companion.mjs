...
async function buildSetupReport(cwd, actionsTaken = []) {
  ...
  if (!opencodeStatus.available) {
    nextSteps.push("Install opencode with `npm install -g opencode-ai` or `curl -fsSL https://opencode.ai/install | bash`.");
  }
  if (opencodeStatus.available && !authStatus.loggedIn && authStatus.requiresOpenaiAuth) {
    nextSteps.push("Run `!opencode auth login`.");
    nextSteps.push("If browser login is blocked, see `opencode auth login --help` for alternatives.");
  }
...
function ensureOpencodeAvailable(cwd) {
  ...
  if (!availability.available) {
    throw new Error(
      "opencode CLI is not installed or not on PATH. Install it with `npm install -g opencode-ai` (or `curl -fsSL https://opencode.ai/install | bash`), then rerun `/opencode:setup`."
    );
  }
}
...
async function executeReviewRun(request) {
  ...
  const prompt = buildReviewPrompt(
    isAdversarial ? "adversarial-review" : "review",
    reviewName,
    context,
    focusText
  );
...
async function handleCancel(argv) {
  ...
  const interrupt = await interruptOpencodeTurn(cwd, { threadId, turnId });
  if (interrupt.attempted) {
    appendLogLine(
      job.logFile,
      interrupt.interrupted
        ? `Requested opencode session abort for ${turnId} on ${threadId}.`
