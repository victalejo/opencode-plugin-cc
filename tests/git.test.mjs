...
import { collectReviewContext, resolveReviewTarget } from "../plugins/opencode/scripts/lib/git.mjs";
...
test("collectReviewContext skips broken untracked symlinks instead of crashing", { skip: process.platform === "win32" ? "symlink creation requires elevated privileges on Windows" : false }, () => {
  const cwd = makeTempDir();
  initGitRepo(cwd);
  fs.writeFileSync(path.join(cwd, "app.js"), "console.log('v1');\n");
  run("git", ["add", "app.js"], { cwd });
