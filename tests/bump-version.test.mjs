...
function makeVersionFixture() {
  ...
  writeJson(path.join(root, "plugins", "opencode", ".claude-plugin", "plugin.json"), {
    name: "opencode",
    version: "1.0.2"
  });
...
test("bump-version updates every release manifest", () => {
  ...
  assert.equal(readJson(path.join(root, "plugins", "opencode", ".claude-plugin", "plugin.json")).version, "1.2.3");
...
test("bump-version check mode reports stale metadata", () => {
  ...
  assert.match(result.stderr, /plugins\/opencode\/\.claude-plugin\/plugin\.json version/);
